...
export function InventoryReportDetail({ isTab = false }: { isTab?: boolean }) {
    ...
    return (
        <div className={cn(
            "space-y-8 text-foreground transition-colors duration-500 max-w-[2400px] mx-auto",
            isTab ? "" : "p-8 min-h-screen"
        )}>
            {/* Header Area */}
            {!isTab ? (
                ...
                <div className="flex justify-end">
                    <button
                        onClick={exportToExcel}
                        disabled={!reportData}
                        className="h-11 px-6 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl font-black text-xs uppercase tracking-widest shadow-lg shadow-emerald-900/20 transition-all active:scale-[0.98] disabled:opacity-50 flex items-center gap-2"
                    >
                        <Download className="w-4 h-4 ml-1" />
                        تصدير Excel
                    </button>
            ...
            <div className="glass-card bg-card/40 backdrop-blur-xl border border-border/50 rounded-3xl p-6 shadow-2xl relative overflow-hidden group">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6 items-end relative z-10">
                    ...
                    <div className="space-y-3">
                        ...
                        <Input
                            type="number"
                            className="bg-background/40 border-border/40 h-12 rounded-2xl focus:ring-cyan-500/20 transition-all font-bold placeholder:text-muted-foreground/30"
                            placeholder="مثال: 5"
                            ...
                            <table className="w-full text-sm">
                                <thead className="bg-muted/50 border-b border-border/40">
                                    <tr>
                                        ...
                                        <th className="text-center py-4 px-6 text-[10px] font-black text-foreground/60 uppercase tracking-widest">الكمية المتاحة</th>
                                    ...
                                    {filteredProducts.length > 0 ? (
                                        filteredProducts.map((p: any) => (
                                            <tr key={p.id} className="transition-all hover:bg-primary/10 even:bg-muted/70 group h-14">
                                                ...
                                                <td className="py-2 px-6 text-foreground/60 text-xs font-bold">{p.category}</td>
                                        ...
                                        <tr>
                                            <td colSpan={9} className="py-12 text-center text-foreground/30 font-bold uppercase tracking-widest text-xs">
